...
class TestBitmapEditor < Test::Unit::TestCase
  ...
  # Test the new_canvas functions
  def test_new_canvas
    x = BitmapEditor.new(nil)
    assert_equal("You haven't created an canvas yet", x.process("S"))
    assert_equal("", x.process("I 5 6"))
    assert_equal("OOOOO\nOOOOO\nOOOOO\nOOOOO\nOOOOO\nOOOOO", x.process("S"))
    assert_equal("The canvas must be at minimum 1x1", x.process("I 0 9"))
  ...
  # Test the clear_canvas function
  def test_clear_canvas
    x = BitmapEditor.new(nil)
    assert_equal("You haven't created an canvas yet", x.process("C"))
  ...
  def test_set_pixel_colour
    ...
    assert_equal("You haven't created an canvas yet", x.process("L 1 2 3"))
    assert_equal("", x.process("I 4 5"))
    assert_equal("Colours must be capital letters", x.process("L 1 2 3"))
    assert_equal("", x.process("L 1 2 K"))
    assert_equal("OOOO\nKOOO\nOOOO\nOOOO\nOOOO", x.process("S"))
    assert_equal("You entered a pixel which isn't in the canvas", x.process("L 0 4 X"))
    assert_equal("You entered a pixel which isn't in the canvas", x.process("L 7 2 Z"))
  ...
  def test_draw_line
    x = BitmapEditor.new(nil)
    assert_equal("You haven't created an canvas yet", x.process("H 3 4 9 J"))
    assert_equal("You haven't created an canvas yet", x.process("V 8 5 2 A"))
  ...
  def test_further
    ...
    assert_equal("You haven't created an canvas yet", x.process("s23"))
